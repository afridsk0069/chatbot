...
# Set up Gemini API
os.environ['GOOGLE_API_KEY'] = 'AIzaSyA-H2_OxOQOn2gpgpwtz5F3Eomxj_D2fC0'
...
st.markdown("""
    <style>
        /* General page background and font settings */
        body {
...
def query_healthcare_assistant(symptoms, age_category):
    prompt = f"""
    Given the following symptoms: {symptoms}, list possible conditions and tailored medical advice for a {age_category}.
...
def create_pdf(report):
    ...
    # Adding report content with better spacing between sections
...
# Analyze Symptoms and Find Nearby Facilities
if st.button("Analyze Symptoms and Find Nearby Facilities"):
    if symptoms and address and age_category:
        with st.spinner("Analyzing your symptoms and finding nearby facilities..."):
            # Progress bar for better animation
            progress = st.progress(0)
            for i in range(1, 101):
                progress.progress(i)
                time.sleep(0.05)  # Simulating a delay for the animation effect
            # Symptom analysis
            report = query_healthcare_assistant(symptoms, age_category)
